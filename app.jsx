...
function GuideView(){
  ...
  async function loadGuide(){
    try {
      ...
      // Fallback: prova a caricare comunque da file
      try {
        const response = await fetch('./GUIDA-DETTAGLIATA.md');
        const text = await response.text();
        setContent(text);
        ...
        setContent("⚠️ Errore caricamento guida. Assicurati che GUIDA-DETTAGLIATA.md sia presente.");
        setLoading(false);
      }
    }
  }
  ...
  async function resetGuide(){
    ...
    try {
      ...
      if (window.firebaseDB) {
        const db = window.firebaseDB;
        const docRef = db.collection('userData').doc('guide-content');
        await docRef.set({content: "", lastModified: ""});
      }
